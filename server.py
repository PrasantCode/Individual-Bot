...
from aitrends import run_aitrends_digest, AITRENDS_TRIGGERS, run_aitrends_story_detail, AITRENDS_DETAIL_PATTERN
...
# ---------------------------------------------------------------------------
MIN_BARS      = 20   # lowered so newer IPOs (Ola, Swiggy, etc.) still show data
RSI_OVERSOLD  = 25
VOL_REV_SPIKE = 30
VOL_BRK_SPIKE = 50
...
def _compute_stock_data(symbol: str) -> dict | None:
    ...
    try:
        ...
        # 52-week high / low from fast_info (most reliable source)
        week52_high = float(fi.get("year_high") or fi.get("fiftyTwoWeekHigh") or 0)
        week52_low  = float(fi.get("year_low")  or fi.get("fiftyTwoWeekLow")  or 0)

        # Sector from info (slower but cached after first call)
        try:
            info   = tk.info
            sector = info.get("sector") or info.get("sectorDisp") or ""
            # Fallback 52w from info if fast_info gave 0
            if week52_high == 0:
        ...
        # Try 2y first; newer IPO stocks (Ola, Swiggy, etc.) may have less
        # history — fall back to "max" so we get every available bar.
        df = yf.download(ticker_str, period="2y", interval="1d",
                         progress=False, auto_adjust=True)
        if df is None or len(df) < MIN_BARS:
            df = yf.download(ticker_str, period="max", interval="1d",
                             progress=False, auto_adjust=True)
        if df is None or len(df) < MIN_BARS:
            return None

        # FIX: handle MultiIndex columns that yfinance sometimes returns
        ...
        # Fallback 52w from history if API gave 0
        if week52_high == 0:
        ...
        # Volume spike: 5-day vs 20-day (at EOD row) — safe for small histories
        v5        = float(volume.rolling(5).mean().iloc[eod_idx])  if n_bars >= 5  else 0.0
        v20       = float(volume.rolling(20).mean().iloc[eod_idx]) if n_bars >= 20 else 0.0
        vol_ratio = ((v5 / v20) - 1) * 100 if v20 else 0.0

        # EMAs at EOD row — EMA50/200 marked None when insufficient data
        ema20  = float(close.ewm(span=20,  adjust=False).mean().iloc[eod_idx])
        ema50  = float(close.ewm(span=50,  adjust=False).mean().iloc[eod_idx]) if n_bars >= 50  else None
        ema200 = float(close.ewm(span=200, adjust=False).mean().iloc[eod_idx]) if n_bars >= 100 else None

        # Support / Resistance — use available bars (up to 20 sessions)
        sr_window         = min(20, n_bars - 1)
        res_slice         = high.iloc[eod_idx - sr_window : eod_idx]
        sup_slice         = low.iloc[eod_idx - sr_window : eod_idx]
        recent_resistance = float(res_slice.max()) if len(res_slice) else eod_close
        recent_support    = float(sup_slice.min()) if len(sup_slice) else eod_close
        ...
        return {
            ...
            "vs_ema50":       ("above" if eod_close > ema50  else "below") if ema50  is not None else None,
            "vs_ema200":      ("above" if eod_close > ema200 else "below") if ema200 is not None else None,
...
def _format_stock_message(data: dict) -> str:
    """
    Render the Telegram message — always shows OHLC + 52-week range.
    """
    mode   = data["mode"]
    ticker = data["ticker"]
    signal = data["signal"]
    ...
    # ── Price block — always shown (LIVE + POST) ─────────────────────────────
    price_block = (
        f"⏰  {data['quote_time']}\n"
        f"💰  LTP:           ₹{data['ltp']:.2f}\n"
    ...
    # ── 52-week range block ──────────────────────────────────────────────────
    w52_block = (
        f"🏔️  52W High:    ₹{data['week52_high']:.2f}\n"
        f"🏔️  52W Low:     ₹{data['week52_low']:.2f}\n"
    )
...
STOCK_HELP = (
    ...
    "Examples: *RELIANCE*, *Tata Motors*, *INFY*, *Ola Electric*, *BSE*\n\n"
    "You'll get:\n"
    ...
    "Send *aitrends* for AI trends digest\\."
)
...
async def telegram_ainews(request: Request) -> Response:
    ...
    Triggers:
      ...
      - 'aitrends' / 'trends' / 'ai trends' / 'what are people building' → AI trends digest
      - 'trends 1', 'aitrends 3', etc → detailed view of specific trend item
    ...
    # AI News triggers
    if text_lower in AINEWS_TRIGGERS:
        _an_get_clients()
        threading.Thread(
    ...
    # AI Trends triggers
    elif text_lower in AITRENDS_TRIGGERS:
        _an_get_clients()
        threading.Thread(
            target=run_aitrends_digest, args=(chat_id, 24), daemon=True
        ).start()
    
    # AI Trends detail pattern (e.g., "trends 1", "aitrends 3")
    elif match := AITRENDS_DETAIL_PATTERN.match(text):
        story_number = int(match.group(1))
        threading.Thread(
            target=run_aitrends_story_detail, args=(story_number, chat_id), daemon=True
        ).start()
...
async def telegram_stock(request: Request) -> Response:
    ...
    # AI trends command — redirect user to the correct bot
    if tl in AITRENDS_TRIGGERS:
        _stock_reply(chat_id, "For AI trends, please use the AI News bot\\.")
        return Response(content="ok")
